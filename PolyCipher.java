...
public class PolyCipher extends Cipher {
    ...
    // Our default constructor, accepts a key
    public PolyCipher(String k) {
        this.key = PolyCipher.clean(k);
        this.alphabet = getAlphabet(new String[] {"lower"});
        this.square = new char[alphabet.size()][alphabet.size()];
    }

    // Can get different versions of the alphabet
    public PolyCipher(String k, String[] names) {
        this.key = PolyCipher.clean(k);
        this.alphabet = getAlphabet(names);
        this.square = new char[alphabet.size()][alphabet.size()];
    }

    // Creates a polycipher with the passed through beta matrix char[][]
    public PolyCipher(String k, char[][] beta) {
        this.key = PolyCipher.clean(k);
        this.alphabet = getAlphabet(new String[] {"lower"});
        this.square = beta;
    }
    ...
    /**
         * Encrypts plaintext using the Beta Matrix retrieved via getBeta().
         * 
         * @param plaintext The plaintext to be encrypted
         * @return The encrypted text, created using our key and matrix.
         */
    @Override
    public String encrypt(String plaintext) {
        String output = "";
        plaintext = PolyCipher.clean(plaintext);
        ...
        for(char c : plaintext.toCharArray()) {
            ...
            int plaintextIndex = convert(c);
            char cipherChar = currentCipher[plaintextIndex];
            output += cipherChar;
    ...
    /**
     * Prints the beta matrix for this PolyCipher object.
     */
    public void printSquare() {
        for (int row = 0; row < square.length; row++) {
            for (int col = 0; col < square.length; col++) {
                System.out.print(this.square[row][col]);
            }
            System.out.println();
        }
    }
    ...
    /**
         * Converts a character into the required number for indexing our beta matrix. 
         * Uses the default alphabet: abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?'";:/
         * 
         * @param character The character to be converted to a number based on it's index in our default alphabet
         * @return The number representation of our character
         */
    public int convert(char character) {
        String defaultAlphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?'\";:/";
        return defaultAlphabet.indexOf(character);
    }
    
    /**
         * Converts a number into the required character for indexing our beta matrix. 
         * Uses the default alphabet: abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?'";:/
         * 
         * @param number The character to be converted to a number based on it's index in our default alphabet
         * @return The character representation of our number
         */
    public char convert (int number) {
        String defaultAlphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?'\";:/";
        return defaultAlphabet.charAt(number);
    }

    /**
     * Retrieve the beta matrix from matrix.csv. Stores it to this.square
     * 
     * @return The matrix array. Optional, still set's this.square like a void.
     */
    public char[][] getBeta() {
        // The array that will hold our output.
        char[][] output = new char[71][71];
    ...
    /**
     * Cleans whitespaces from plaintext.
     * @param plainText
     * @return The plaintext without any spaces
     */
    public static String clean(String plainText) {
        return plainText.replaceAll("\\s+", "");
    }

    /**
    * Writes the beta matrix from this.square to matrix.csv. Lines of the matrix should appear in the csv exactly as they do when printed.
    * (Same number of rows and columns, a square)
    */
    public void storeBeta() {
        String outputName = "./matrix.csv"; // Name of the file that stores our random beta matrix

        // Attempts to write to the csv
        try (PrintWriter writer = new PrintWriter(new FileWriter(outputName))) {
            // Write each row of the matrix to matrix.csv
            for (int row = 0; row < square.length; row++) {
                char[] betaRow = this.square[row];
                writer.println(betaRow); // Writes a row of the matrix to the csv output
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    /**
     * Returns the key this cipher uses to encrypt and decrypt.
